...
public class Main extends Application {
    ...
    static boolean[][] maze = new boolean[SIZE][SIZE]; //false = wall
    static Cell start;
    static Cell finish;
    ...
    private static void printAll(List<Cell> path) {
        ...
        for (int r = 0; r < SIZE; r++) {
            for (int c = 0; c < SIZE; c++) {
                ...
                } else root.add(getCell(Color.GREY), c, r);
            }
        }
    ...
    private static List<Cell> getFinallPath() {
        int count = 0;
    ...
    public static List<Cell> getPossibleDirections(Cell currentCell) {
    ...
    private static void initMaze() {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            if (Math.random() < 0.5) {
                ...
                for (int offset = 1; offset <= corridorSize; offset++) {
                ...
                //left - right
                int corridorSize = (int) (Math.random() * 5 + 2);
                int sign = Math.random() < 0.5 ? -1 : 1;
                for (int offset = 1; offset <= corridorSize; offset++) {
                    int row = random.row;
